                   ...
                   const float* const filter, const int filterWidth)
{
  ...
  for (int i=-filterWidth/2; i<=filterWidth/2; ++i){
    for (int j=-filterWidth/2; j<=filterWidth/2; ++j){
      int tx = min(max(ix+i, 0), static_cast<int>(numRows - 1));
      int ty = min(max(iy+j, 0), static_cast<int>(numCols - 1));
      int fx = filterWidth/2 + i;
      int fy = filterWidth/2 + j;
      sum += (float)inputChannel[tx*numCols+ty] * filter[fx*filterWidth+fy];
    }
  }
  outputChannel[ix*numCols+iy] = sum;
}
                      ...
                      unsigned char* const blueChannel)
{
  ...
  int ref_idx = ix * numCols + iy;
  redChannel[ref_idx] = inputImageRGBA[ref_idx].x;
  greenChannel[ref_idx] = inputImageRGBA[ref_idx].y;
  blueChannel[ref_idx] = inputImageRGBA[ref_idx].z;
}
